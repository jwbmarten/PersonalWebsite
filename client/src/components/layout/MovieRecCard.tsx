  ...
  return (
    <div className="flex w-full max-w-3xl flex-col m-6 p-3 text-sm text-white rounded-xl backdrop-blur-sm shadow-lg ring-2 ring-gray-900 bg-white/5">
      ...
      <div className="mt-2 flex items-stretch gap-4">
        ...
        <div className="flex-[0_0_46%] h-40 sm:h-52 rounded-md overflow-hidden flex items-center justify-center">
          {artSrc ? (
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          {/* Top meta */}
          <div>
            <TextMarquee className="text-sm text-gray-300" text={movie?.director ? movie.director : "Unknown Director"} />
            {/* first two genres as chips */}
            {genres.length > 0 && (
              <div className="mt-1 flex flex-wrap gap-2">
                {genres.map((g) => (
                  <span key={g} className="inline-block text-[11px] px-2 py-0.5 rounded-full bg-white/10 border border-white/15">
                    {g}
                  </span>
                ))}
              </div>
          ...
          <div className="mt-auto pt-2">
            ...
            <div className="flex items-center gap-4 justify-start">
              {movie?.imdb_url && (
                <a href={movie.imdb_url} target="_blank" rel="noopener noreferrer" aria-label="Open on IMDb" className="inline-flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 hover:scale-105 transition-transform">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={ImdbIcon} className="w-10 h-10 sm:w-11 sm:h-11 object-contain" alt="IMDb" />
                </a>
              )}
              {movie?.letterboxd_url && (
                <a href={movie.letterboxd_url} target="_blank" rel="noopener noreferrer" aria-label="Open on Letterboxd" className="inline-flex items-center justify-center w-10 h-10 sm:w-11 sm:h-11 hover:scale-105 transition-transform">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={LetterboxdIcon} className="w-10 h-10 sm:w-11 sm:h-11 object-contain" alt="Letterboxd" />
                </a>
              )}
            </div>
            {/* Next row */}
